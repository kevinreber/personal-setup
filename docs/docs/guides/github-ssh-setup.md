...
If you generate new keys, add the public keys to GitHub:
```bash
...
git config core.sshCommand     # Should show personal SSH key path

# Test work directory
cd ~/Documents/code/some-work-repo
git config user.email          # Should show: kreber@linkedin.com
git config core.sshCommand     # Should show work SSH key path
...
1. Git loads the main `~/.gitconfig` first (with work defaults)
...
- [ ] Create `~/.gitconfig` with work defaults and conditional include

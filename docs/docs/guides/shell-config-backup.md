...
./shell-config/install-backup-service.sh run
```
...
1. **Backup Script** (`backup-configs.sh`)
   ...
   - Regenerates the Brewfile from installed Homebrew packages
...
```bash
./shell-config/backup-configs.sh --dry-run
```
...
| Log | Location |
|-----|----------|
| Backup log | `shell-config/.backup.log` |
